...
First, you must install the ridigbio package. If you are new to R and R studio, please refer to our QUBES module to get started (Introduction to R with Biodiversity Data, doi:[10.25334/84FC-TE88](doi.org/10.25334/84FC-TE88)).
...
Here the `rq`, or record query, indicates we want to download all the records where the `scientificname` is equal to [<i>Galax urceolata</i>](https://en.wikipedia.org/wiki/Galax). 
...
The media search above retained `r nrow(galax_media)` rows, however some of these observations do not have information in the `accessuri` field. To only obtain records with `acessuri`, we indicate we only want records where `data.ac:accessURI` exist, by setting `mq`, or media query, as followed:
```{r}
galax_media2 <- idig_search_media(rq=list(scientificname="Galax urceolata"),
                                  mq=list("data.ac:accessURI"=list("type"="exists")))
```

Now we have  `r nrow(galax_media2)` observations with `accessuri`!